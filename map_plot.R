# Get the map of paris
mapParis <- borders("france","paris", colour="gray50", fill="gray50")
mpP <- ggplot() + mapParis

# Plot the 5 top districts for filming in paris on the map
...
# Plot the top 5 tourist attractions in paris on the map
...
mpP + labs(x = "Coordinates long") + labs(y = "Coordinates lat") + theme(axis.title.x = element_text(size = rel(2))) + theme(axis.title.y = element_text(size = rel(2))) + theme(axis.text = element_text(size = rel(1.5)))